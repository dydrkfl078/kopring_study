...
class StrategyV1Test {

    val strategy = StrategyImpl()

    @Test
    @DisplayName("전략패턴 V1 - 구현 클래스를 사용한다.")
    fun strategyTestV1() {
        val strategy1 = StrategyContextV1(strategy)
    ...
    fun strategyTestV2() {
        val strategy2 = StrategyContextV1(
            object : Strategy {
                override fun call() {
    ...
    fun strategyTestV3(){
        val strategy3 = StrategyContextV1 { logger.info { "비즈니스 로직 3번 실행" } }
        strategy3.execute()

        val strategy4 = StrategyContextV1 { logger.info { "비즈니스 로직 4번 실행" }}
        strategy4.execute()
    }
}